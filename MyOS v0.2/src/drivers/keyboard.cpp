...
KeyBoardDriver::KeyBoardDriver(InterruptManager* manager, KeyboardEventHandler* handler) 
    ...
    dataport(0x60),            //键盘中断 0x60,读写缓冲区，读写0x60端口
    commandport(0x64),         //读状态寄存器，读0x64端口，控制寄存器，写0x64端口
    handler(handler) {
...
void KeyBoardDriver::Activate() {                   //键盘开启
...
uint32_t KeyBoardDriver::HandleInterrupt(uint32_t esp) {
    ...
    case 0x2A: case 0x36: shift = true; break;    // 左 shift，按下对应 0x2A 0x36
    case 0xAA: case 0xB6: shift = false; break;   // 左 shift，弹起对应 0xAA 0xB6 
