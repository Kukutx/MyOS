...
uint32_t PeripheralComponentInterconnectController::Read(uint8_t bus, 
    ...
    uint32_t id = 
        ...
        ((function & 0x07) << 8) |
        (registeroffset & 0xfc);
    commandPort.Write(id);   // 往 command 端口发送地址
    ...
    for (uint16_t bus = 0; bus < 256; bus++) {
        for (uint8_t device = 0; device < 32; device++) {
            ...
            for (uint8_t function = 0; function < numFunctions; function++) {
                ...
                printf("PCI BUS ");
                printfHex(bus & 0xff);                    // bus 取低位第 1 个字节，7..0
                ...
                printf(" = VENDOR ");
                printfHex((dev.vendor_id & 0xff00) >> 8);  // vendor_id 取低位第 2 个字节，15..8
